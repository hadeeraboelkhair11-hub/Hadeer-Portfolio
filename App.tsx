...
const InvitationRoute = ({ slug }: { slug: string }) => {
  const initial = slug === 'demo' ? royalBurgundySeed : getInvitationBySlug(slug);
  const [invitation, setInvitation] = useState<Invitation | undefined>(initial);
  useEffect(() => {
    if (slug === 'demo') return;
